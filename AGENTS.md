...
Name content files with lowercase, hyphen-separated names such as `bash-history.md` or `json-tools.md`. Keep examples copyable and prefer explicit language tags on code fences, for example ` ```bash ` or ` ```json `. Use Geekdoc shortcodes, such as `{{< toc >}}`, only when they improve navigation or readability.
...
Follow repository-specific guidance in `CLAUDE.md` as additional context. Do not add attribution lines such as `Co-Authored-By` or `Signed-off-by` to commits.
